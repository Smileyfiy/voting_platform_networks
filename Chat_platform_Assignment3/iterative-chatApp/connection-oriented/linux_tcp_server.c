/*
 * TCP Chat Server - Kali Linux
 * Iterative Connection-Oriented Chat Application
 *
 * Compile:  gcc tcp_server.c -o tcp_server
 * Run:      ./tcp_server <port>
 * Example:  ./tcp_server 9000
 *
 * NOTE: Iterative means the server handles ONE client fully
 * before accepting the next connection. Other connecting
 * clients wait in the kernel's accept() backlog queue.
 *
...
#define BACKLOG 5 /* pending connections queue length */

/* ── Client record ── */
typedef struct
{
    int fd; /* connected socket fd, -1 = inactive */
...
/* Send a full string over a TCP socket (handles partial sends) */
...
/* ── Handle one connected client (called from the iterative loop) ── */
static void handle_client(int client_fd, struct sockaddr_in *addr)
{
    ...
    /* Register slot immediately so broadcasts reach this client */
...
int main(int argc, char *argv[])
{
    ...
    /* Reuse address so restart doesn't hit TIME_WAIT */
    int opt = 1;
    ...
    /* Listen — key TCP step absent in UDP */
    ...
    printf("=== TCP Chat Server (Kali Linux) on port %d ===\n", port);
